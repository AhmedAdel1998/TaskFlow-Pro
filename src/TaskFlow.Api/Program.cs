...
app.MapGet("/api/push/vapid-public-key",(IConfiguration cfg)=>{var k=cfg["Vapid:PublicKey"];return string.IsNullOrEmpty(k)?Results.NotFound():Results.Ok(new{publicKey=k});});
app.MapPost("/api/push/subscribe",async(ClaimsPrincipal p,PushSubscribeCommand x,IPushSubscriptionService s,CancellationToken ct)=>{await s.SubscribeAsync(User(p),x,ct);return Results.NoContent();}).RequireAuthorization();
app.MapPost("/api/push/unsubscribe",async(ClaimsPrincipal p,UnsubscribeRequest x,IPushSubscriptionService s,CancellationToken ct)=>{await s.UnsubscribeAsync(User(p),x.Endpoint,ct);return Results.NoContent();}).RequireAuthorization();
app.Run();
static Guid User(ClaimsPrincipal p)=>Guid.Parse(p.FindFirst("sub")!.Value); static Guid Org(ClaimsPrincipal p)=>Guid.Parse(p.FindFirst("org")!.Value); public record RefreshRequest(string RefreshToken); public record UnsubscribeRequest(string Endpoint);
